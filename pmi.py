...
def svd_ppmi(corpus, window_length=5, embedding_size=200, content_distribution_smoothing=0.75,
    ...
    ppmi_matrix = get_ppmi_matrix(skipgrams, count_matrix, content_distribution_smoothing)
...
if __name__ == '__main__':
    ...
    from preprocess_text import preprocess_e2e
    import pandas as pd
    print('reading data')
    df = pd.read_csv('../data/reddit/Qsubs/qsub_comments.gzip',
                     compression='gzip',
                     usecols='body id'.split()).dropna(subset=['body']).sample(100000)
    print('preprocessing')
    df['body'] = df.body.apply(preprocess_e2e)
    docs = df.body.values
    print('tokenizing')
    docs = list(map(word_tokenize, docs))
    unigram_counts, id2tok, tok2id, pmis = get_word_embeddings(docs)
